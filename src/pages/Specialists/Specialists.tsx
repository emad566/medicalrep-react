...
import { SpecialistColumns } from "./components/SpecialistColumns";
...
const Specialists = () => {
    ...
    async function handleList(queryData?:any, showLoading=true){
      ...
      const responseJson = await apiGet(Routes.specialists, queryData?? queryParams);
        if (responseJson.status) {
            setData(responseJson.data.specialists);
        }else{
    ...
    return (
        <Fragment>
          <TableCaption
            ...
            parent={t(AppLangKeys.specialists)}
            formTitle={""}
            isCreate={false} 
