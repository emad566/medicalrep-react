...
export const PermissionColumns = (page, perPage, handleDelete, toggleHandler) => {
...
return [
  {
    name: t("ID"),
    selector: (row, index) => {
        return <div key={`${row.id}`}>{((page-1) * perPage) + index + 1}</div>;
    },
    sortable: false,
    center: true,
    width: "5%",
  ...
  {
    name: t("Roles"),
    selector: (row) => <div key={`${row.id}`} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }} className='pt-1'>{row.roles.map((permission)=><span key={`${row.id}${permission}${uuidv4()}`} className="badge rounded-pill badge-warning mb-1">{permission}</span>)}</div>,
    sortable: false,
    center: false,
    width: "40%",
    class: ""
